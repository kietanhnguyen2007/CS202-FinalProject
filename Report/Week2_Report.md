...
- **Backend Event Hookups:** Successfully integrated sound triggers into the core engine without altering existing physics/combat logic. Hooked up Player inputs, Enemy/Boss states, Pet actions (summon, dragon fire, ghost heal), and the `ElementalSystem` reactions (Vaporize, Conduct, Overload).

**25125037 – Nguyen Trong Tien**
- **UI & View Audio Integration:** Connected interface and game state audio triggers (e.g., Level Complete, Game Over) within `ResultView` and `MenuController` to provide immediate feedback to the player.
- **Repository Optimization:** Removed multiple heavy, unused dummy `.wav` files from the repository to drastically reduce the project footprint and optimize git clone times.
- **Build System Updates:** Updated `CMakeLists.txt` and `.gitignore` to accommodate the new structured audio directories and prevent massive `.ogg`/`.wav` binaries from clogging the source control.
